...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Band'))
            conn.commit()
    conn.close()
...
def addToList(self):
    ...
    if not "@" or not "." in var_email: # come back to this
...
def onUpdate(self):
    try:
        ...
        return
    # The user will only be allowed to update changes for phone, emails, and course.
