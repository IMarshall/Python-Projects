...
def load_gui(self):
    ...
    self.lstList1.bind('<<ListboxSelect>>', lambda event: StudentTrackerFUNC.onSelect(self,event))
    ...
    self.btn_add = tk.Button(self.master, width=12, height=2, text='Add', command=lambda: StudentTrackerFUNC.addToList(self))
    self.btn_add.grid(row=10, column=0, padx=(25,0), pady=(45,10), sticky=W)
    self.btn_update = tk.Button(self.master, width=12, height=2, text='Update', command=lambda: StudentTrackerFUNC.onUpdate(self))
    self.btn_update.grid(row=10, column=1, padx=(15,0), pady=(45,10), sticky=W)
    self.btn_delete = tk.Button(self.master, width=12, height=2, text='Delete', command=lambda: StudentTrackerFUNC.onDelete(self))
    self.btn_delete.grid(row=10, column=2, padx=(15,0), pady=(45,10), sticky=W)
    self.btn_close = tk.Button(self.master, width=12, height=2, text='Close', command=lambda: StudentTrackerFUNC.ask_quit(self))
    self.btn_close.grid(row=10, column=4, columnspan=1, padx=(15,0), pady=(45,10), sticky=E)

    StudentTrackerFUNC.create_db(self)
    StudentTrackerFUNC.onRefresh(self)
